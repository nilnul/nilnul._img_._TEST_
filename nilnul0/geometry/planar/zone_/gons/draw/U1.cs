...
namespace nilnul.geometry.planar.zone_.gons.draw
{
	public class U1
	{
		static public string draw(int width = 800,bool hollow=false, params IEnumerable<Complex>[] contour ) {


			var img = drawGons(width,hollow,contour);
		...
		static public string draw(int width = 800,bool nonfill=false, params IEnumerable<Point4dblI>[] contour ) {


			var img = drawGons(width,nonfill,contour);
		...
		static public void show(int width ,  IEnumerable<IEnumerable<Complex>> contour,bool hollow=false ) {

			nilnul.fs.file._ExeX.Exe(
				draw(
					width,hollow,contour.ToArray()
		...
		static public void show(int width = 800,bool nonfill=false, params IEnumerable<Point4dblI>[] contour ) {

			nilnul.fs.file._ExeX.Exe(
				draw(width,nonfill,contour)
			);
		...
		static public Bitmap drawGons(int width, bool hollow, params IEnumerable<Complex>[] contour)
		{
			return drawGons(width, hollow, contour.Select(c => c.Select(d => Point4dbl.Of(d))));
		}

		static public Bitmap drawGons(int width,bool hollow, IEnumerable< IEnumerable<Point4dblI>> contour )
		{
		...
		static public Bitmap drawGons(int width,bool nonfill, params IEnumerable<Point4dblI>[] contour )
		{
			var boundingBox = nilnul.geometry.planar.points_.dwelt._BoundaryX._Boundary_0dwelt(
				contour.SelectMany(x => x)
			);
		...
		static public void drawGons(Graphics g,bool hollow, IEnumerable< IEnumerable<Point4dblI>> contour )
		{
		...
		static public void drawGons(Graphics g,bool nonfill, params IEnumerable<Point4dblI>[] contour )
		{
			var boundingBox = nilnul.geometry.planar.points_.dwelt._BoundaryX._Boundary_0dwelt(contour.SelectMany(x => x));
		...
		public static void Show(Gon4dbl facet,bool fill)
		{
			show(800, facet.cycles,fill);
		}
		...
		public static void Show(IEnumerable<IEnumerable<Complex>> zones,bool hollow=false)
		{
			show(800, zones,hollow);
		}



		public static void Show(IEnumerable<Point4dblI> conduit,bool file=false)
		{
			Show([conduit] );
		}
		...
		public  static void Show(IEnumerable<Complex> zones,bool hollow=false)
		{
			Show([zones],hollow );
